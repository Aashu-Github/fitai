...
def init_db():
    ...
    try:
        c.execute("ALTER TABLE meal_plan ADD COLUMN cost REAL NOT NULL DEFAULT 0.0")
        conn.commit()
    except Exception:
        pass
...
def create_reset_token(email: str) -> dict | None:
    """Generate a reset token for the given email. Returns {token, user_id} or None."""
    import time
    conn = get_db()
...
def consume_reset_token(token: str) -> int | None:
    """Validate token (max 1 hour, unused). Returns user_id or None."""
    import time
    conn = get_db()
    row = conn.execute(
        "SELECT user_id, created_at, used FROM password_reset_tokens WHERE token=?", (token,)
    ).fetchone()
    if not row or row["used"] or (int(time.time()) - row["created_at"]) > 3600:
        conn.close()
        return None
...
def fetch_week_plan(user_id, week) -> dict:
    conn = get_db()
    rows = conn.execute(
        "SELECT day,meal_type,name,cals,cost,emoji FROM meal_plan WHERE user_id=? AND week=?",
        (user_id, str(week))
    ).fetchall()
    conn.close()
    plan = {}
    for row in rows:
        plan.setdefault(row["day"], {})[row["meal_type"]] = {
            "name": row["name"], "cals": row["cals"], "cost": row["cost"], "emoji": row["emoji"]
        }
    return plan

def upsert_meal(user_id, week, day, meal_type, name, cals, cost, emoji) -> dict:
    conn = get_db()
    conn.execute("""INSERT INTO meal_plan (user_id,week,day,meal_type,name,cals,cost,emoji) VALUES (?,?,?,?,?,?,?,?)
        ON CONFLICT(user_id,week,day,meal_type) DO UPDATE SET name=excluded.name,cals=excluded.cals,cost=excluded.cost,emoji=excluded.emoji""",
        (user_id, str(week), day, meal_type, name, cals, cost, emoji))
    conn.commit(); conn.close()
