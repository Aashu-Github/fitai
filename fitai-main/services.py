...
def signup(display_name, username, email, password, confirm_password) -> dict:
    ...
    if not _re.match(r"^[a-zA-Z0-9]+$", username):
        raise ValueError("Username must only consist of letters and numbers.")
    if not password or len(password) < 10 or not _re.search(r"\d", password):
        raise ValueError("Password must be at least 10 characters & have at least one digit.")
