...
def update_profile(user_id, fields) -> dict:
    if "weight" in fields and not str(fields["weight"]).strip():
...
def forgot_password(email: str) -> dict:
    """
    Creates a reset token for the email (if it exists).
    Always returns a success-like message to avoid user enumeration.
    Returns the token so the caller can build a reset link (dev mode).
    In production, swap this for an SMTP send and omit the token from the response.
    """
    result = repo.create_reset_token(email)
    if not result:
        # Don't reveal whether the email exists
        return {"message": "If that email is registered, a reset link has been sent.", "token": None}
    return {"message": "If that email is registered, a reset link has been sent.", "token": result["token"]}

def reset_password(token: str, new_password: str):
    """Consume a valid reset token and update the password."""
    if not new_password or len(new_password) < 10 or not any(c.isdigit() for c in new_password):
        raise ValueError("Password must be at least 10 characters and contain at least one digit.")
    user_id = repo.consume_reset_token(token)
    if not user_id:
...
def save_meal(user_id, week, day, meal_type, entry) -> dict:
    # The frontend sends day as "Monday 1/6" — extract just the day name for validation
    day_name = day.split(" ")[0] if day else ""
    if day_name not in VALID_DAYS: raise ValueError(f"Invalid day: {day}")
    ...
    # Store the full day string (with date) so the frontend can match it back
    plan = repo.upsert_meal(user_id, str(week), day, meal_type,
                            entry["name"], int(entry.get("cals",0)),
                            float(entry.get("cost",0)), entry.get("emoji","🍽️"))
